...
const CapitalMarketsTicket = ({state,onChange,disabled,errors,quote,planImpact,allowedInstruments,title,otherArea}:{state:ActionFormState;onChange:(s:ActionFormState)=>void;disabled?:boolean;errors?:Partial<Record<keyof ActionFormState,string>>;quote?:CapitalMarketsBookbuildResult;planImpact?:CapitalMarketsPlanImpact;allowedInstruments:CapitalMarketsInstrument[];title:string;otherArea:string}) => {
  ...
  return <section className="policy-disclosure capital-markets-ticket" aria-label="Capital markets">
    <div className="policy-section-title"><h3>{title}</h3><small>Market demand sets fill and terms.</small></div>
    ...
    <p className="muted">Terms clear at close. A failed price limit settles nothing.</p>
  </section>;
};
...
export default function ActionsPanel({department,state,onChange,disabled,errors,hasValidationErrors,giltQuotedYield,capitalMarketsQuote,capitalMarketsPlanImpact,maxGiltSaleAmount,maxBoeFundingAmount}:Props) {
  ...
  return <div className="stack department-policy-panel">
    <div className="policy-section-title"><h3>{department==='Treasury'?'Treasury actions':'Standing policy'}</h3><small>{department==='Treasury'?'Cash stays in BoE reserves until invested. Trades execute once.':'Policies persist; one-off orders clear after close.'}</small></div>
    ...
    {department==='Customers'&&<>
      ...
      <details className="department-advanced policy-disclosure">
        ...
        <p className="muted">Fixed-term funding trades a higher rate for stability and a set maturity.</p>
        <div className="policy-fields">
          ...
          <label className="field"><strong>Term</strong><select value={state.termDepositTenorMonths} disabled={disabled} onChange={e=>update('termDepositTenorMonths',e.target.value)}><option value="12">1 year</option><option value="24">2 years</option><option value="36">3 years</option></select><small>Longer terms improve stability but lock in rates.</small></label>
        </div>
      </details>
    </>}
    ...
    {department==='Capital'&&<>
      ...
      <p className="muted">AT1 coupons are discretionary. New issues clear at market rates.</p>
    </>}

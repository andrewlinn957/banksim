...
export default function ActionsPanel({department,state,onChange,disabled,errors,hasValidationErrors,giltQuotedYield}:Props) {
  ...
  return <div className="stack department-policy-panel">
    ...
    {(department==='Treasury'||department==='Capital')&&<button className="button ghost" disabled={disabled} onClick={()=>onChange(department==='Capital'?{...state,issueEquityAmount:'',issueTier2Amount:''}:{...state,giltTradeDirection:'none',giltTradeAmount:'',issueLTDebtAmount:'',boeFacility:'none',boeFundingAmount:'',hedgeDirection:'none',hedgeNotional:''})}>Cancel queued transactions</button>}

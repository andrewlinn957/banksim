...
export default function DepartmentOffice({department,state,history,form,errors,hasErrors,selected,onChange,onDecision,onReport,onHelp,estimate}:Props) {
 ...
 const competitorRates=department==='Customers'
  ? [
    ['Retail savings',state.market.competitorRetailDepositRate],
  ...
  : department==='Lending'
   ? [
     ...
     ['Business loans',state.market.riskFreeLong+state.market.corporateLoanSpread],
    ] as const
   : [];
 return <div className="department-office">
  ...
  {competitorRates.length>0&&<section className="competitor-rates" aria-label="Competitor rates"><div><strong>Competitor rates</strong><small>Current market offers</small></div><dl>{competitorRates.map(([label,rate])=><div key={label}><dt>{label}</dt><dd>{formatPct(rate)}</dd></div>)}</dl></section>}

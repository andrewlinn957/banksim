...
export default function RiskAppetiteEditor({state,config,pending,onQueue}: {state:BankState;config:SimulationConfig;pending?:RiskAppetite|null;onQueue:(t:RiskAppetite|null)=>void}) {
  ...
  const floors = {
    cet1: Math.max(config.riskLimits.minCet1Ratio, state.risk.riskMetrics.cet1Requirement),
    leverage: config.riskLimits.minLeverageRatio,
    lcr: config.riskLimits.minLcr,
    nsfr: config.riskLimits.minNsfr,
  };
  ...
  const validRatios=ratioKeys.every(k=>draft[k]?.trim()!==''&&Number.isFinite(ratioValues[k])&&ratioValues[k]>=floors[k]&&ratioValues[k]<=(k==='cet1'||k==='leverage'?1:10));
  ...
  return <section className="capital-card risk-appetite-editor"><h3>Board risk appetite</h3><p>Set standing targets. CET1 governs distribution restraint; capital and liquidity targets govern safety pauses. The IRRBB limit is used at the next annual Pillar 2A assessment. Targets apply at the next monthly close and persist until changed.</p>
    <div className="appetite-fields">{ratioKeys.map(k=><label className="field" key={k}><strong>{k==='cet1'?'CET1':k==='leverage'?'Leverage':k.toUpperCase()} target (%)</strong><input type="number" step="0.01" min={(floors[k]*100).toFixed(2)} max={k==='cet1'||k==='leverage'?100:1000} value={draft[k]} disabled={state.status.hasFailed} onChange={e=>setDraft({...draft,[k]:e.target.value})}/><small>Minimum allowed: {formatPct(floors[k])}. Current effective target: {formatPct(k==='cet1'?state.risk.riskMetrics.internalCet1TargetRatio:Math.max(floors[k],state.behaviour.riskAppetite?.[k]??automatic[k]))}</small></label>)}
    ...
    <p className="muted">Targets are constrained to the bank's current prudential floors. The IRRBB limit does not change the current P2A immediately: it is one input to the next annual SREP assessment.</p>
    ...
    {!valid&&<p role="alert">Each ratio target must meet its displayed minimum; capital targets may be up to 100%, liquidity targets up to 1,000%, and the IRRBB EVE limit must be positive.</p>}

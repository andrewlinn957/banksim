...
/** Fixed quantitative mandate supplied by the board when optional plan mode is enabled. */
export const createDefaultThreeYearPlanTargets = (state: BankState): ThreeYearPlanTarget[] => {
  ...
  const lending = read(M.customerLending);
  const deposits = read(M.customerDeposits);
  return [
...
export const createDefaultThreeYearPlan = (state: BankState) => createThreeYearPlanState({
  startStep: state.time.step,
  targets: createDefaultThreeYearPlanTargets(state),
  settings: { ...DEFAULT_THREE_YEAR_PLAN_SETTINGS, enabled: true },
});
